...
// Ticker ...
type Ticker struct {
	Mid       decimal.Big `json:"mid"`        // mid (price): (bid + ask) / 2
	Bid       decimal.Big `json:"bid"`        // bid (price): Innermost bid.
	Ask       decimal.Big `json:"ask"`        // ask (price): Innermost ask.
	LastPrice decimal.Big `json:"last_price"` // last_price (price) The price at which the last order executed.
	Low       decimal.Big `json:"low"`        // low (price): Lowest trade price of the last 24 hours
	High      decimal.Big `json:"high"`       // high (price): Highest trade price of the last 24 hours
...
type Fees struct {
	...
	LastUpdatedMs     string      `json:"last_updated_ms"`
	WebMakerFeeBPS    decimal.Big `json:"web_maker_fee_bps"`
	WebTakerFeeBPS    decimal.Big `json:"web_taker_fee_bps"`
	WebAuctionFeeBPS  decimal.Big `json:"web_auction_fee_bps"`
	APIMakerFeeBPS    decimal.Big `json:"api_maker_fee_bps"`
	APITakerFeeBPS    decimal.Big `json:"api_taker_fee_bps"`
	APIAuctionFeeBPS  decimal.Big `json:"api_auction_fee_bps"`
	FixMakerFeeBPS    decimal.Big `json:"fix_maker_fee_bps"`
	FixTakerFeeBPS    decimal.Big `json:"fix_taker_fee_bps"`
	FixAuctionFeeBPS  decimal.Big `json:"fix_auction_fee_bps"`
	BlockMakerFeeBPS  decimal.Big `json:"block_maker_fee_bps"`
	BlockTakerFeeBPS  decimal.Big `json:"block_taker_fee_bps"`
	Notional30DVolume string      `json:"notional_30d_volume"`
	Notional1DVolume  []string    `json:"notional_1d_volume"`
}
...
// CancelOrder cancels an offer given its id.
...
func (api *API) NewOrder(currency string, amount string, price decimal.Big, isBuy bool, clientOrderID string) (order OrderStatus, error ErrorMessage) {
...
func (api *API) Fees() (fee Fees, error ErrorMessage) {
	request := map[string]interface{}{
...
func (api *API) Heartbeat() (heartbeat Heartbeat) {
	request := map[string]interface{}{
